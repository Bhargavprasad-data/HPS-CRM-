...
export const login = createAsyncThunk('auth/login', async (creds, { rejectWithValue }) => {
  try {
    ...
    if (user.role !== 'Staff') return rejectWithValue('Access denied. This portal is for Staff only.');
...
const authSlice = createSlice({
  name: 'auth',
  initialState: { user: stored ? JSON.parse(stored) : null, isAuthenticated: !!localStorage.getItem('hps_access_token'), loading: false, error: null },
  reducers: {
