...
api.interceptors.request.use((config) => { const token = localStorage.getItem('hps_access_token'); if (token) config.headers.Authorization = `Bearer ${token}`; return config; });
