...
import {
  LayoutDashboard, Users, FolderKanban, ListTodo, UserCheck,
  Ticket, BarChart3, LogOut, Briefcase, Menu, X, ChevronRight,
  Sun, Moon, Bell, Activity, Clock
} from 'lucide-react';
...
const NAV = [
  ...
  { path: '/tasks',     label: 'Tasks Pipeline',    icon: ListTodo        },
      ...
      >
        ...
        <AnimatePresence>
          {(expanded || mobile) && (
            ...
            >
              <span className="text-teal-400 font-extrabold text-sm leading-none tracking-wider">HPS</span>
        ...
        >
          {user?.photo ? (
            <img
              src={user.photo}
              alt="Profile"
              className="w-8 h-8 rounded-xl object-cover shadow-md shrink-0 ring-2 ring-teal-500/0 group-hover:ring-teal-500/40 transition-all"
            />
          ) : (
            <div className="w-8 h-8 bg-gradient-to-br from-teal-600 to-emerald-600 rounded-xl flex items-center justify-center text-white font-bold text-sm shadow-md shrink-0 ring-2 ring-teal-500/0 group-hover:ring-teal-500/40 transition-all">
          ...
          <AnimatePresence>
            {(expanded || mobile) && (
              ...
              >
                <p className="text-[var(--text-primary)] text-xs font-semibold truncate leading-tight">{user?.name || 'Manager'}</p>
                <p className="text-teal-400 text-[10px] truncate mt-0.5">View Profile →</p>
              </motion.div>
            )}
          </AnimatePresence>
          <AnimatePresence>
...
export default function ManagerLayout({ children }) {
  ...
  const [theme,      setTheme]      = useState(() => localStorage.getItem('hps_mgr_theme') || 'dark');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('hps_mgr_theme', theme);
  }, [theme]);
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[var(--bg-main)] text-[var(--text-primary)]">
      ...
      {/* ── Desktop Sidebar ── Fixed w-16 anchor keeps content stable */}
        ...
        >
          ...
          <div className="hidden sm:flex items-center gap-2 text-sm text-[var(--text-secondary)]">
            ...
            <span className="text-[var(--text-primary)] font-semibold">
              {currentNav?.label || 'Manager Portal'}
            </span>
          </div>
          ...
          {/* Role badge */}
          <div className="flex items-center gap-2 px-3 py-1.5 bg-teal-500/10 border border-teal-500/20 rounded-xl select-none">
            <div className="w-2 h-2 rounded-full bg-teal-400 dot-pulse" />
            <span className="text-teal-400 text-xs font-bold uppercase tracking-wider">Manager</span>
          </div>
          ...
          {user?.photo ? (
            <Link to="/profile" title="My Profile" className="shrink-0">
              <img
                src={user.photo}
                alt="Profile"
                className="w-8 h-8 rounded-xl object-cover shadow-md cursor-pointer select-none ring-2 ring-teal-500/0 hover:ring-teal-500/40 transition-all"
              />
            </Link>
          ) : (
            <Link to="/profile" title="My Profile" className="shrink-0">
              <div className="w-8 h-8 bg-gradient-to-br from-teal-600 to-emerald-600 rounded-xl flex items-center justify-center text-white font-bold text-sm shadow-md cursor-pointer select-none ring-2 ring-teal-500/0 hover:ring-teal-500/40 transition-all">
                {user?.name?.[0]?.toUpperCase() || 'M'}
              </div>
            </Link>
          )}
