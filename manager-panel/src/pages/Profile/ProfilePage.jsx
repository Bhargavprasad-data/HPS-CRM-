  ...
  return (
    <div className="space-y-6 pb-6">
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(135deg, var(--bg-card) 60%, rgba(20,184,166,0.07) 100%)',
          borderColor: 'rgba(20,184,166,0.15)',
        }}
      >
        <div className="absolute inset-y-0 right-0 w-72 bg-gradient-to-l from-teal-500/5 to-transparent pointer-events-none" />

        <div className="flex flex-col md:flex-row items-center gap-6 relative z-10">
          {/* Avatar with Upload */}
          <div className="relative shrink-0 group">
            <div className="w-24 h-24 rounded-3xl overflow-hidden shadow-xl shadow-teal-500/20 ring-2 ring-teal-500/20">
              {displayPhoto ? (
                <img
                  src={displayPhoto}
                  alt="Profile"
                ...
                <div className="w-full h-full bg-gradient-to-br from-teal-600 to-emerald-600 flex items-center justify-center text-white font-bold text-4xl">
                  {p.name?.[0]?.toUpperCase() || user?.name?.[0]?.toUpperCase() || 'M'}
                </div>
              )}
            </div>
          ...
          <div className="text-center md:text-left space-y-1.5 flex-1">
            ...
            <p className="text-sm flex items-center justify-center md:justify-start gap-1.5" style={{ color: 'var(--text-secondary)' }}>
              ...
              {p.designation || 'Manager'} · {p.department || 'Department'}
            </p>
            <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
              Employee ID: <span className="font-mono text-teal-400">{p.employee_code || 'HPS-XXX'}</span>
            </p>
          </div>
        ...
        {photoBase64 && (
          <div className="mt-5 flex items-center gap-3 p-3 rounded-xl bg-teal-500/10 border border-teal-500/20">
            <Camera className="w-4 h-4 text-teal-400 shrink-0" />
            <p className="text-sm text-teal-300 flex-1">New photo selected — click Save to upload</p>
            <button
            ...
            <button
              onClick={cancelPhoto}
              className="p-1.5 rounded-lg text-teal-400 hover:text-teal-200 hover:bg-teal-500/20 transition-colors"
              title="Cancel"
            >
        ...
        <div className="lg:col-span-1 space-y-5">
          ...
          <div className="card space-y-5">
            <h3 className="font-semibold flex items-center gap-2 border-b pb-3" style={{ color: 'var(--text-primary)', borderColor: 'var(--border-card)' }}>
              <User className="w-4 h-4 text-teal-400" /> Personal Details
            </h3>
          ...
          <div className="card space-y-5">
            <h3 className="font-semibold flex items-center gap-2 border-b pb-3" style={{ color: 'var(--text-primary)', borderColor: 'var(--border-card)' }}>
              <CreditCard className="w-4 h-4 text-teal-400" /> Financial Records
            </h3>
            <div className="space-y-3 text-sm">
              {[
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="card">
            <h3 className="font-semibold mb-5 flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
              <BadgeCheck className="w-4 h-4 text-teal-400" /> Edit Account Details
            </h3>
            <form onSubmit={handleProfileSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="col-span-1 sm:col-span-2 flex justify-end mt-1">
                ...
                >
                  {profileLoading ? <Loader2 className="w-4 h-4 animate-spin text-teal-500" /> : <Save className="w-4 h-4" />}
          ...
          <div className="card">
            <h3 className="font-semibold mb-5 flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
              <KeyRound className="w-4 h-4 text-teal-400" /> Change Password
            </h3>
            <form onSubmit={handlePasswordSubmit} className="space-y-4">
              ...
              <div className="flex justify-end mt-1">
                <button
                  ...
                  className="btn-primary flex items-center gap-2"
                >
                  {passLoading ? <Loader2 className="w-4 h-4 animate-spin text-teal-500" /> : <Lock className="w-4 h-4" />}
