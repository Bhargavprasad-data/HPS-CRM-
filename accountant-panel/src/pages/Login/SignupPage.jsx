...
const DEPARTMENTS = [
  'Finance', 'Sales', 'IT & Development',
  'Marketing', 'HR', 'Operations', 'Customer Support',
];

const BENEFITS = [
  { icon: Shield,       text: 'Secure & encrypted financial data'    },
  { icon: Users,        text: 'Collaborative accountant workspace'   },
...
export default function SignupPage() {
  ...
  const [form, setForm] = useState({
    ...
    phone: '', department: 'Finance', designation: '',
  });
  ...
  const strengthColor  = ['', '#ef4444', '#f97316', '#f59e0b', '#10b981'][strength];
  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'var(--bg-main)', color: 'var(--text-primary)' }}>
      ...
      {/* ── Left Panel – Branding ─────────────────────────── */}
      <div
        className="hidden lg:flex flex-col justify-between w-[38%] relative overflow-hidden p-12"
        style={{ background: 'linear-gradient(135deg, #0c0700 0%, #1c0f00 60%, #3d1a00 100%)' }}
      >
        {/* Decorative orbs */}
        ...
        <div className="relative z-10">
          ...
          <p className="text-amber-200/60 text-base leading-relaxed mb-10">
            Register your accountant account to access the premium finance management portal.
          </p>
          ...
          <div className="space-y-3">
            {BENEFITS.map(({ icon: Icon, text }) => (
              <div key={text} className="flex items-center gap-3">
                ...
                <p className="text-amber-200/80 text-sm">{text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── Right Panel – Form ──────────────────────────────── */}
      <div
        className="flex-1 flex flex-col items-center justify-center p-6 lg:p-12 overflow-y-auto"
        ...
        <div className="w-full max-w-lg">
          ...
          <div className="mb-6">
            ...
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
              Fill in your details to join the HPS Accountant portal
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              <label className="relative cursor-pointer shrink-0">
                <div
                  className="w-16 h-16 rounded-2xl overflow-hidden flex items-center justify-center transition-all"
                  style={{
                    background:  photoPreview ? 'transparent' : 'var(--bg-hover)',
                    border: `2px dashed ${photoPreview ? 'transparent' : 'rgba(217,119,6,0.35)'}`,
                  }}
                >
                  {photoPreview
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field label="Designation" icon={Briefcase}>
                <input className="input pl-10" type="text" placeholder="Senior Accountant"
              ...
              <div className="flex items-center gap-2">
                {[...Array(4)].map((_, i) => (
                  <div
                    ...
                    style={{ background: i < strength ? strengthColor : 'var(--border-card)' }}
                  />
                ))}
                <span className="text-xs shrink-0" style={{ color: strengthColor }}>
                  {strengthLabel}
                </span>
              </div>
            )}
